...
public interface ThreadRepository extends JpaRepository<Thread, Long> {
    ...
    @Query("SELECT t FROM Thread t WHERE t.forum.active = true ORDER BY t.createdAt DESC")
    Page<Thread> findRecentThreads(Pageable pageable);
    
    @Query("SELECT t FROM Thread t WHERE t.forum.active = true AND (LOWER(t.title) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(t.content) LIKE LOWER(CONCAT('%', :query, '%')))")
